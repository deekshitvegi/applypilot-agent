...
(function () {
  ...
  const PLACEHOLDER = /^(|-+|\.+|_+|no selection|none selected|nothing selected|not selected|select\b.*|please select.*|choose\b.*|pick one|--.*--|\(.*\)|click to select.*|type to search.*|start typing.*|search\.\.\.|n\/?a|tbd|optional|required)$/i;
